...
const episodes = async (titleId: string, queryStr = '') => {
  try {
    // https://www.imdb.com/title/tt0364343/episodes?season=1
    ...
    const has_more_episodes = $main.find('.ipc-see-more').attr('class') !== undefined;

    return { meta, episodes, has_more_episodes };
  } catch (err) {
